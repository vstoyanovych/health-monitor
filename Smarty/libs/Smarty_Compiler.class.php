...
class Smarty_Compiler extends Smarty {
    ...
    function _compile_file($resource_name, $source_content, &$compiled_content)
    {
        ...
        $special_blocks_callback = function ($matches) {
            return $this->_quote_replace($this->left_delimiter)
                . 'php' . str_repeat("\n", substr_count($matches[1], "\n"))
                . $this->_quote_replace($this->right_delimiter);
        };
        $source_content = preg_replace_callback($search, $special_blocks_callback, $source_content);
        ...
        if (sm_strlen($compiled_content) && (substr($compiled_content, -1) == "\n") ) {
...
function _smarty_sort_length($a, $b)
{
    ...
    if(sm_strlen($a) == sm_strlen($b))
        return ($a > $b) ? -1 : 1;

    return (sm_strlen($a) > sm_strlen($b)) ? -1 : 1;
}
