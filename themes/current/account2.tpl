{if $modules[$index].mode eq "show" or $modules[$index].mode eq "wronglogin"}
    {if $modules[$index].panel eq "center"}
        ...
        <div class="view-header view-login-block"  style="{if $modules[$index].mode eq "wronglogin"}display:block{/if}">
        ...
        <div class="panel panel-filled view-choose-account-block"  style="{if $modules[$index].mode eq "wronglogin"}display:none{/if}">
            <div class="panel-body">
                <div class="user-choose-account-another row">
                    <div class="fa fa-user col-xs-2 another-user-icon" aria-hidden="true"></div><h5> Use another account</h5>
                </div>
            </div>
        </div>

    <div class="panel panel-filled view-login-block" style="{if $modules[$index].mode eq "wronglogin"}display:block{/if}">
        <div class="panel-body">
                ...
                <div class="form-group">
                    <div class="flex w-100">
                        ...
                        {if $_settings.allow_forgot_password eq "1"}
                            <a href="index.php?m=resetpassword">{$lang.forgot_password_question}</a>
                        {/if}
                    </div>
                </div>
